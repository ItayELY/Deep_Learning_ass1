...
print(sys.argv)
# Flags:
mlpn = len(sys.argv) > 1 and "n" in sys.argv[1]
if mlpn:
    import mlpn as mlp
    print("Using mlpn.")
    layers = [int(s) for s in sys.argv[2].split(";")]
else:
    import mlp1 as mlp
test = "t" in sys.argv
...
if __name__ == '__main__':
    ...
    if not only_unigram:
        ...
        L2I, F2I = u.L2I, u.F2I
        if mlpn:
            print("*** Training bigram mlpn. ***")
            params = mlp.create_classifier(layers)
        else:
